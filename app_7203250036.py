...
def tampilkan_bastp(transaksi_list):
    st.markdown("""
        <div class="dashboard-card">
            <h3 style="margin-top:0; color:#065f46; font-size:18px;">📋 Pratinjau, Cetak & Download Berita Acara Selesai Pekerjaan (BASTP - Multi-Item Ready)</h3>
        </div>
    ...
    with col_sel2:
        lokasi_office = st.text_input("📍 Lokasi Office (Tempat BASTP):", value="Luwuk", key="bastp_lokasi")
    ...
    # Tanggal Utama BASTP
    selected_date = st.date_input("📅 Tanggal Utama Berita Acara Selesai (BASTP):", value=date(2026, 7, 31), key="bastp_main_date")
    bulan_indo = {
    ...
    st.markdown("---")
    st.markdown("#### ⚙️ Pengaturan Parameter Detail per Baris Pekerjaan BASTP")
    ...
    for idx, m in enumerate(mutasi_terpilih, start=1):
        ...
        with c_b1:
            row_date = st.date_input(f"Selesai Operasi (Item {idx})", value=date(2026, 7, 31), key=f"bastp_date_{selected_pi}_{idx}")
        with c_b2:
            row_qty = st.number_input(f"Jumlah / Qty (Item {idx})", min_value=0.0, value=float(m.get('Qty', 1.0)), step=1.0, format="%.2f", key=f"bastp_qty_{selected_pi}_{idx}")
        with c_b3:
            default_uom = str(m.get('Unit', 'Month')).strip()
            default_idx = uom_options.index(default_uom) if default_uom in uom_options else 0
            row_uom = st.selectbox(f"Satuan (Item {idx})", uom_options, index=default_idx, key=f"bastp_uom_{selected_pi}_{idx}")
        with c_b4:
            row_catatan = st.text_input(f"Catatan (Item {idx})", value="", placeholder="Opsional", key=f"bastp_cat_{selected_pi}_{idx}")
    ...
    st.markdown("---")
    st.markdown("#### 🖼️ Pengaturan Logo Header Dokumen BASTP")
    c_log1, c_log2 = st.columns(2)
    with c_log1:
        uploaded_logo_1 = st.file_uploader("Upload Logo Pihak Pertama", type=["png", "jpg", "jpeg"], key="logo_bastp_1")
