...
def tampilkan_pemantauan_pembayaran():
    st.markdown(
        "#### 📊 Modul Analisis Keuangan, Pemantauan Pembayaran & Aging Invoice"
    )
    ...
    def ambil_grand_total_invoice_master(inv_no):
        for inv in invoice_list:
            found_no = str(
                inv.get("Nomor Invoice Resmi", inv.get("Nomor Invoice", ""))
            ).strip()
    ...
    EXCEL_PAYMENT_STATUS = os.path.join(
        DIR_DATABASE, "database_status_pembayaran.xlsx"
    )
                ...
                if df is not None and not df.empty:
                    ...
                    for r in records:
                        ...
                        if master_gt > 0:
                            ...
                            r["Grand Total"] = parse_harga_presisi(
                                r.get("Grand Total", 0.0)
                            )
                    return records
            except:
                pass
    ...
    if not invoice_list and not payment_records:
        st.warning(
            "⚠️ Belum ada Data Invoice Resmi atau Status Pembayaran yang tersimpan di direktori."
        )
        return
    ...
    def ambil_tanggal_invoice(inv_data_obj):
        for k, v in inv_data_obj.items():
            if any(
                kata in str(k).lower() for kata in ["tgl", "tanggal", "date"]
            ) and not any(
                kata in str(k).lower()
                for kata in ["penyerahan", "tempo", "lunas"]
            ):
    ...
    all_contracts = sorted(
        list(
            dict.fromkeys(
                [
                    str(
                        inv.get("Kontrak No.", inv.get("Nomor Kontrak", "-"))
                    ).strip()
                    for inv in invoice_list
                    if inv.get("Kontrak No.") or inv.get("Nomor Kontrak")
                ]
            )
        )
    )
    if not all_contracts and payment_records:
        all_contracts = sorted(
            list(
                dict.fromkeys(
                    [
                        str(p.get("Nomor Kontrak", "-")).strip()
                        for p in payment_records
                        if p.get("Nomor Kontrak")
                    ]
                )
            )
        )
    ...
    filter_kontrak_pilih = st.selectbox(
        "Pilih Nomor Kontrak untuk Filter Dashboard:",
        opsi_filter_kontrak,
        key="filter_kontrak_dashboard",
    )

    if filter_kontrak_pilih != "-- Semua Nomor Kontrak (ALL) --":
        filtered_payment_records = [
            p
            for p in payment_records
            if str(p.get("Nomor Kontrak", "")).strip()
            == str(filter_kontrak_pilih).strip()
        ]
    ...
    if payment_records:
        ...
        for p in filtered_payment_records:
            ...
            try:
                dt_jt = datetime.strptime(
                    str(p.get("Tanggal Jatuh Tempo"))[:10], "%Y-%m-%d"
                ).date()
        ...
        persen_dibayar = (
            (total_sudah_dibayar / total_seluruh_tagihan * 100)
            if total_seluruh_tagihan > 0
            else 0.0
        )
        persen_sisa = (
            (sisa_belum_terbayar / total_seluruh_tagihan * 100)
            if total_seluruh_tagihan > 0
            else 0.0
        )
        ...
        st.markdown("---")
        st.markdown(
            f"##### 💰 Rekapitulasi Saldo Keuangan & Tagihan ({filter_kontrak_pilih})"
        )
        ...
        for rh, rht in zip(rh_cols, r_headers):
            with rh:
                st.markdown(
                    f"<span style='font-size: 11px; font-weight: bold; color: #0f172a;'>{rht}</span>",
                    unsafe_allow_html=True,
                )
        st.markdown(
            "<hr style='margin: 4px 0; border-top: 2px solid #cbd5e1;'>",
            unsafe_allow_html=True,
        )
        ...
        for c_key, c_val in summary_contract_map.items():
            ...
            st_teks = (
                "🟢 Lengkap"
                if c_val["lunas"] == c_val["jml_inv"]
                else f"🟡 {c_val['lunas']}/{c_val['jml_inv']} Lunas"
            )
            ...
            with rc_cols[1]:
                st.markdown(
                    f"<small>{c_val['jml_inv']} Dok</small>",
                    unsafe_allow_html=True,
                )
            with rc_cols[2]:
                st.markdown(
                    f"<small>{fmt_rp(t_tag)}</small>", unsafe_allow_html=True
                )
            with rc_cols[3]:
                st.markdown(
                    f"<small>{fmt_rp(t_byr)}</small>", unsafe_allow_html=True
                )
            with rc_cols[4]:
                st.markdown(
                    f"<small>{fmt_rp(t_piu)}</small>", unsafe_allow_html=True
                )
            with rc_cols[5]:
                st.markdown(
                    f"<small><b>{pct_real:.2f}%</b></small>",
                    unsafe_allow_html=True,
                )
            with rc_cols[6]:
                st.markdown(
                    f"<small>{st_teks}</small>", unsafe_allow_html=True
                )
            st.markdown(
                "<hr style='margin: 2px 0; border-top: 1px solid #e2e8f0;'>",
                unsafe_allow_html=True,
            )

        tot_pct_overall = (
            (tot_sum_terbayar / tot_sum_tagihan * 100)
            if tot_sum_tagihan > 0
            else 0.0
        )
        ...
        with tot_cols[3]:
            st.markdown(
                f"**{fmt_rp(tot_sum_terbayar)}**", unsafe_allow_html=True
            )
        with tot_cols[4]:
        ...
        st.markdown(
            "<hr style='margin: 4px 0; border-top: 2px solid #0f172a;'>",
            unsafe_allow_html=True,
        )

        st.markdown("---")
        st.markdown(
            "##### 📈 Grafik Analisis Komparasi Keuangan & Persentase Kinerja Penagihan"
        )
        ...
        st.markdown("---")
        st.markdown(
            "##### 🚨 Indikator Peringatan & Aging Status Pembayaran (Sistem Notifikasi)"
        )
    ...
    st.markdown("---")
    st.markdown(
        "##### 📝 Form Input & Pembaruan Status Pembayaran (Berdasarkan Kontrak)"
    )

    col_fc1, col_fc2 = st.columns([1.5, 2.5])
    with col_fc1:
        form_kontrak_pilih = st.selectbox(
            "1️⃣ Pilih Nomor Kontrak:",
            all_contracts if all_contracts else ["-"],
            key="form_input_kontrak_sel",
        )
    ...
    inv_list_filtered_contract = [
        ...
        == str(form_kontrak_pilih).strip()
    ]

    sample_inv = invoice_list[0] if invoice_list else {}
    inv_key = (
        "Nomor Invoice Resmi"
        if "Nomor Invoice Resmi" in sample_inv
        else (
            "Nomor Invoice"
            if "Nomor Invoice" in sample_inv
            else (list(sample_inv.keys())[0] if sample_inv else "Nomor Invoice")
        )
    )

    all_inv_no_contract = [
        str(inv.get(inv_key, "")).strip()
        for inv in inv_list_filtered_contract
        if inv.get(inv_key)
    ]
    if not all_inv_no_contract and payment_records:
        all_inv_no_contract = sorted(
            list(
                dict.fromkeys(
                    [
                        str(p.get("Nomor Invoice", "")).strip()
                        for p in payment_records
                        if str(p.get("Nomor Kontrak", "")).strip()
                        == str(form_kontrak_pilih).strip()
                        and p.get("Nomor Invoice")
                    ]
                )
            )
        )
    ...
    saved_invoice_set = {
        str(p.get("Nomor Invoice", "")).strip()
        for p in payment_records
        if p.get("Nomor Invoice")
    }

    active_edit_inv = str(
        st.session_state.get("active_invoice_selected", "")
    ).strip()
    list_inv_aktif = [
        inv_no
        for inv_no in all_inv_no_contract
        if inv_no not in saved_invoice_set or inv_no == active_edit_inv
    ]
    if not list_inv_aktif and all_inv_no_contract:
        list_inv_aktif = all_inv_no_contract

    list_saved_payment_no = sorted(
        list(
            {
                str(p.get("Nomor Invoice"))
                for p in payment_records
                if str(p.get("Nomor Kontrak")) == str(form_kontrak_pilih)
            }
        ),
        reverse=True,
    )
    opsi_panggil_bayar = [
        "-- Pilih Data Tersimpan untuk Diedit / Panggil Ulang --"
    ] + list_saved_payment_no

    with col_fc2:
        pilihan_panggil_bayar = st.selectbox(
            "2️⃣ Panggil Ulang Data Pemantauan Tersimpan (Kontrak Terpilih):",
            opsi_panggil_bayar,
            key="select_panggil_bayar",
        )
        if st.button("📥 Panggil untuk Diedit", use_container_width=True):
            if (
                pilihan_panggil_bayar
                != "-- Pilih Data Tersimpan untuk Diedit / Panggil Ulang --"
            ):
                st.session_state["active_invoice_selected"] = str(
                    pilihan_panggil_bayar
                ).strip()
                st.success(
                    f"📋 Memuat data pemantauan Invoice `{pilihan_panggil_bayar}`"
                )
                st.rerun()
    ...
    if not list_inv_aktif:
        selected_inv = st.text_input(
            "3️⃣ Ketik Nomor Invoice Aktif:", value=active_edit_inv
        )
    else:
        selected_inv = st.selectbox(
            "3️⃣ Pilih Nomor Invoice Aktif:",
            list_inv_aktif,
            index=(
                default_select_idx
                if default_select_idx < len(list_inv_aktif)
                else 0
            ),
            key="dropdown_master_invoice_aktif",
        )
    ...
    inv_data = next(
        (
            inv
            for inv in invoice_list
            if str(inv.get(inv_key, "")).strip() == str(selected_inv)
        ),
        {},
    )
    existing_pay = next(
        (
            p
            for p in payment_records
            if str(p.get("Nomor Invoice", "")).strip() == str(selected_inv)
        ),
        {},
    )

    tgl_invoice_bawaan = (
        ambil_tanggal_invoice(inv_data)
        if inv_data
        else str(existing_pay.get("Tanggal Invoice", date.today()))[:10]
    )
    ...
    status_pembayaran = st.selectbox(
        "Status Pembayaran:",
        status_opsi,
        index=idx_st,
        key="select_status_pembayaran_live",
    )
    ...
    with st.form("form_update_pembayaran"):
        ...
        with col_p1:
            ...
            nomor_faktur_pajak = st.text_input(
                "Nomor Faktur Pajak (Diterbitkan setelah Invoice):",
                value=str(default_faktur),
            )

            default_tgl_serah = datetime.today().date()
            if existing_pay.get("Tanggal Penyerahan"):
                try:
                    default_tgl_serah = datetime.strptime(
                        str(existing_pay.get("Tanggal Penyerahan"))[:10],
                        "%Y-%m-%d",
                    ).date()
                except:
                    pass
            tgl_penyerahan = st.date_input(
                "Tanggal Invoice Diserahkan ke Klien:", value=default_tgl_serah
            )

            default_top = int(existing_pay.get("TOP Hari", 30))
            top_hari = st.number_input(
                "Term of Payment (TOP dalam Hari):",
                min_value=0,
                value=default_top,
                step=5,
            )
        ...
        with col_p2:
            ...
            raw_tgl_lunas_exist = str(
                existing_pay.get("Tanggal Pelunasan", "-")
            )
            ada_tgl_lunas_exist = (
                raw_tgl_lunas_exist != "-"
                and raw_tgl_lunas_exist.strip() != ""
            )
            ...
            if status_pembayaran in ["Sebagian (DP / Termin)", "Lunas"]:
                ...
                if ada_tgl_lunas_exist:
                    try:
                        default_tgl_lunas = datetime.strptime(
                            raw_tgl_lunas_exist[:10], "%Y-%m-%d"
                        ).date()
                    except:
                        pass
                tgl_pelunasan = st.date_input(
                    "Tanggal Pelunasan Aktual:", value=default_tgl_lunas
                )
            else:
                st.markdown(
                    "📅 Tanggal Pelunasan Aktual: **... (Belum Ada Pembayaran / Kosong)**"
                )
                tgl_pelunasan = None

        catatan_bayar = st.text_area(
            "Catatan / Keterangan Pembayaran:",
            value=str(existing_pay.get("Catatan", "")),
        )

        submit_simpan = st.form_submit_button(
            "💾 Simpan Pemantauan Pembayaran",
            type="primary",
        ...
        if submit_simpan:
            if not selected_inv.strip():
                ...
                if (
                    status_pembayaran in ["Sebagian (DP / Termin)", "Lunas"]
                    and tgl_pelunasan
                ):
                ...
                data_update = {
                    ...
                    "Customer": inv_data.get(
                        "Customer", existing_pay.get("Customer", "-")
                    ),
                    "Tanggal Invoice": tgl_invoice_bawaan,
                    ...
                    "Update Terakhir": datetime.today().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                }
                ...
                clean_records = [
                    p
                    for p in payment_records
                    if str(p.get("Nomor Invoice", "")).strip()
                    != str(selected_inv).strip()
                ]
                clean_records.append(data_update)
                simpan_status_pembayaran(clean_records)
                st.success(
                    f"🎉 Berhasil menyimpan data pemantauan untuk Invoice [{selected_inv}]!"
                )
    ...
    st.markdown("---")
    st.markdown(
        f"#### 📋 Ringkasan & Laporan Aging Invoice ({filter_kontrak_pilih})"
    )

    current_payment_records = muat_status_pembayaran()
    if filter_kontrak_pilih != "-- Semua Nomor Kontrak (ALL) --":
        current_payment_records = [
            p
            for p in current_payment_records
            if str(p.get("Nomor Kontrak", "")).strip()
            == str(filter_kontrak_pilih).strip()
        ]
    ...
    if current_payment_records:
        hdr_cols = st.columns(
            [1.1, 1.4, 1.3, 1.8, 0.9, 0.9, 0.6, 0.9, 0.9, 1.1, 1.0, 0.7, 0.7]
        )
        headers_text = [
            "No. Kontrak",
            "No. Invoice",
        ...
        for hc, ht in zip(hdr_cols, headers_text):
            with hc:
                st.markdown(
                    f"<span style='font-size: 11px; font-weight: bold; color: #0f172a;'>{ht}</span>",
                    unsafe_allow_html=True,
                )
        st.markdown(
            "<hr style='margin: 4px 0; border-top: 2px solid #cbd5e1;'>",
        ...
        for idx, row_p in enumerate(current_payment_records):
            ...
            if tgl_pelunasan_raw and tgl_pelunasan_raw != "-":
                try:
                    dt_serah = datetime.strptime(
                        tgl_penyerahan_str, "%Y-%m-%d"
                    ).date()
                    dt_lunas = datetime.strptime(
                        tgl_pelunasan_raw[:10], "%Y-%m-%d"
                    ).date()
                    selisih_riil = (dt_lunas - dt_serah).days
                    durasi_info = (
                        f"{row_p.get('TOP Hari', 0)}d (R: {selisih_riil}d)"
                    )
                except:
                    pass
            ...
            with cols_r[0]:
                st.markdown(
                    f"<small>{row_p.get('Nomor Kontrak', '-')}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[1]:
            ...
            with cols_r[2]:
                st.markdown(
                    f"<small>{faktur_pajak_row if faktur_pajak_row else '-'}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[3]:
                st.markdown(
                    f"<small>{row_p.get('Customer', '-')}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[4]:
                st.markdown(
                    f"<small>{str(row_p.get('Tanggal Invoice', ''))[:10]}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[5]:
                st.markdown(
                    f"<small>{tgl_penyerahan_str}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[6]:
                st.markdown(
                    f"<small>{durasi_info}</small>", unsafe_allow_html=True
                )
            with cols_r[7]:
                st.markdown(
                    f"<small>{str(row_p.get('Tanggal Jatuh Tempo', ''))[:10]}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[8]:
                st.markdown(
                    f"<small>{tgl_pelunasan_str}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[9]:
                gt_val = float(row_p.get("Grand Total", 0))
                st.markdown(
                    f"<small>Rp {gt_val:,.2f}</small>"
                    .replace(",", "X")
                    .replace(".", ",")
                    .replace("X", "."),
                    unsafe_allow_html=True,
            ...
            with cols_r[10]:
                st.markdown(
                    f"<small>{row_p.get('Status Pembayaran', '-')}</small>",
                    unsafe_allow_html=True,
                )
            with cols_r[11]:
            ...
            if st.session_state.get(f"confirm_del_{idx}", False):
                st.error(
                    f"⚠️ Konfirmasi Keamanan: Masukkan Password Admin untuk menghapus Invoice [{inv_num_row}]"
                )
                pass_input = st.text_input(
                    f"Password Verifikasi ({inv_num_row}):",
                    type="password",
                    key=f"pwd_del_{idx}",
                )
                ...
                with col_vk1:
                    if st.button("✔️ Konfirmasi", key=f"btn_yes_del_{idx}"):
                        if pass_input in ["bss2026", "admin123", "admin"]:
                            ...
                            updated_recs = [
                                p
                                for p in all_master_recs
                                if str(p.get("Nomor Invoice", "")).strip()
                                != str(inv_num_row).strip()
                            ]
                            simpan_status_pembayaran(updated_recs)
                            st.success(
                                f"🗑️ Data pemantauan Invoice [{inv_num_row}] berhasil dihapus!"
                            )
        ...
        st.info(
            "ℹ️ Belum ada data pemantauan pembayaran yang tersimpan untuk filter kontrak ini."
        )