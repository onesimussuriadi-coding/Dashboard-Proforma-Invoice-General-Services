...
def tampilkan_pemantauan_pembayaran():
  ...
  # Menggunakan Absolute Path berbasis lokasi file ini agar tidak pernah salah baca folder
  BASE_DIR = os.path.dirname(os.path.abspath(__file__))
  PROJECT_ROOT = os.path.dirname(BASE_DIR)
  DIR_DATABASE = os.path.join(PROJECT_ROOT, "database_penyimpanan_aman")

  if not os.path.exists(DIR_DATABASE):
    # Fallback jika folder berada langsung sejajar dengan working directory
  ...
  if not invoice_list and not payment_records:
    st.warning(
        "⚠️ Belum ada Data Invoice Resmi atau Status Pembayaran yang"
  ...
  def ambil_tanggal_invoice(inv_data_obj):
    for k, v in inv_data_obj.items():
      if any(kata in str(k).lower() for kata in ["tgl", "tanggal", "date"]) and not any(
          kata in str(k).lower() for kata in ["penyerahan", "tempo", "lunas"]
      ):
  ...
  if filter_kontrak_pilih != "-- Semua Nomor Kontrak (ALL) --":
    ...
    filtered_invoice_list = [
        inv
        for inv in invoice_list
        if str(inv.get("Kontrak No.", inv.get("Nomor Kontrak", "-"))).strip()
        == str(filter_kontrak_pilih).strip()
    ]
  else:
  ...
  if payment_records:
    ...
    for rh, rht in zip(rh_cols, r_headers):
      with rh:
        st.markdown(
            f"<span style='font-size: 11px; font-weight: bold; color:"
            f" #0f172a;'>{rht}</span>",
            unsafe_allow_html=True,
        )
    st.markdown(
    ...
    for c_key, c_val in summary_contract_map.items():
      ...
      with rc_cols[5]:
        st.markdown(
            f"<small><b>{pct_real:.2f}%</b></small>", unsafe_allow_html=True
        )
      with rc_cols[6]:
    ...
    st.markdown("---")
    st.markdown(
        "##### 📈 Grafik Analisis Komparasi Keuangan & Persentase Kinerja"
        " Penagihan"
    )
    ...
    st.markdown("---")
    st.markdown(
        "##### 🚨 Indikator Peringatan & Aging Status Pembayaran (Sistem"
        " Notifikasi)"
    )
  ...
  # --- STATUS PEMBAYARAN DI LUAR FORM AGAR REAKTIF SEKETIKA ---
  ...
  with st.form("form_update_pembayaran"):
      ...
      if existing_pay.get("Tanggal Penyerahan"):
        try:
          default_tgl_serah = datetime.strptime(
              str(existing_pay.get("Tanggal Penyerahan"))[:10], "%Y-%m-%d"
          ).date()
        except:
          pass
    ...
    with col_p2:
      ...
      ada_tgl_lunas_exist = (
          raw_tgl_lunas_exist != "-" and raw_tgl_lunas_exist.strip() != ""
      )
      ...
      if status_pembayaran in ["Sebagian (DP / Termin)", "Lunas"]:
        ...
        st.markdown(
            "📅 Tanggal Pelunasan Aktual: **... (Belum Ada Pembayaran /"
            " Kosong)**"
        )
        tgl_pelunasan = None
    ...
    if submit_simpan:
      if not selected_inv.strip():
        ...
        st.success(
            "🎉 Berhasil menyimpan data pemantauan untuk Invoice"
            f" [{selected_inv}]!"
        )
  ...
  if current_payment_records:
    ...
    for idx, row_p in enumerate(current_payment_records):
      ...
      with cols_r[12]:
        if st.button(
            "🗑️", key=f"tbl_del_{idx}_{inv_num_row}", help="Hapus Data"
        ):
      ...
      if st.session_state.get(f"confirm_del_{idx}", False):
        st.error(
            f"⚠️ Konfirmasi Keamanan: Masukkan Password Admin untuk menghapus"
            f" Invoice [{inv_num_row}]"
        )
        ...
        with col_vk1:
          if st.button("✔️ Konfirmasi", key=f"btn_yes_del_{idx}"):
            if pass_input in ["bss2026", "admin123", "admin"]:
              ...
              st.success(
                  f"🗑️ Data pemantauan Invoice [{inv_num_row}] berhasil"
                  " dihapus!"
              )
    ...
    st.info(
        "ℹ️ Belum ada data pemantauan pembayaran yang tersimpan untuk filter"
        " kontrak ini."
    )