...
# --- SISTEM KEAMANAN LOGIN KORPORAT INTERNAL ---
def periksa_login():
    ...
    if not st.session_state.logged_in:
        st.markdown("""
            <div style="max-width: 420px; margin: 60px auto; padding: 30px; background: #ffffff; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border-top: 5px solid #065f46;">
                <h3 style="text-align: center; color: #065f46; margin-top: 0;">🔐 Internal Corporate Login</h3>
                <p style="text-align: center; font-size: 13px; color: #475569; font-weight: 500;">PT Banggai Sentral Sulawesi - Akses Terbatas Internal</p>
            </div>
        """, unsafe_allow_html=True)
        
        with st.form("login_form"):
            username_input = st.text_input("Username / Email Korporat")
            ...
            if submit_btn:
                ...
                if username_input.strip().lower() == "admin" and password_input == "bss2026":
                    ...
                    st.error("⚠️ Username atau Password salah. Silakan coba kembali.")
        return False
    return True

# Jalankan proteksi login sebelum memuat aplikasi utama
if periksa_login():
